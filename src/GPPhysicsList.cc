...
{	
  ...
  dCutElectron=1.0e-3;
  dCutPositron=1.0e-3;
  sPolarizedFlag="unpolarized";
  pPhysicsConstructor= new GPPhysicsPolarized(sPolarizedFlag);
...
void GPPhysicsList::SetParameter(std::string sGlobal)
{
  ...
  else if(sKey=="polarize")
  {
    sPolarizedFlag=sValueOrg;
  }
  else 
  {
